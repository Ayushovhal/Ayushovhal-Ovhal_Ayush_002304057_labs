...
public class AnalysisHelper {
    ...
    //Q2 - Post with most liked comments
    ...
    public void getPostWithMostLikes(){
        DataStore data = DataStore.getInstance();
        Post postWithMostComment = null;
        for(Post p: data.getPosts().values()){
            if(postWithMostComment == null){
                postWithMostComment = p;
            }
            if(p.getComments().size() > postWithMostComment.getComments().size()){
                postWithMostComment = p;
            }
        }
        System.out.println("Q3 - Post with Most Comments"+ postWithMostComment.getPostId());
    }
    
    
    //Q4 - Top 5 inactive users based on total number of posts
    public void passiveUsers(){
        DataStore data = DataStore.getInstance();
    ...
    public void getPassiveCommentUsers(){
        ...
        for(int i = 0; i<5; i ++){
            System.out.println(users.get(i) + ", - Comments count: " + commentNumbers.get(users.get(i).getId()));
        }
    }
    
    
    public void getPassiveAndActiveUsers(){
        DataStore data = DataStore.getInstance();
